...
class MultiModal(nn.Module):
    ...
    def forward(self, input_ids, all_hidden_states, image_raw, text, image, image_generation, image_relative):  # 模型的输入是Bert的最后的隐藏层和所有隐藏层，原图，clip的图文表征。  表示文本的bert和clip分别是all_hidden_states和text
        ...
        x_gg = x_gg.sum(1).view(batch_size_v, batch_size_t, 64)
        x_rr = torch.stack(x_rr, dim=1)
        x_rr = x_rr.sum(1).view(batch_size_v, batch_size_t, 64)
        sim_m = self._calculsim(x_mm) # 计算相似度，输入bs,bs,1024，输出bs,bs，即成对图片和文本之间的相似度值
        sim_g = self._calculsim(x_gg)
        sim_r = self._calculsim(x_rr)
